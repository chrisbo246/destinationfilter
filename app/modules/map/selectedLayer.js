...
$.when(deferred.ready.map, deferred.load.mapSettings).then(function () {
    ...
    function selectedLayerActions(layer) {
        // 'use strict';
        // var raster = layer;
    ...
    function vectorLayerActions(layer) {
        // 'use strict';
        var vector = layer;
    ...
    function imageryLayerActions(layer) {
        // 'use strict';
        var imagery = layer;
    ...
    // 'use strict';
    if (!map) {
        console.warn('map is not defined'); /* RemoveLogging:skip */
    }
    ...
    baseLayerInput.addEventListener('change', function () {
        ol.control.LayerSwitcher.forEachRecursive(map, function (layer) { // l, i, a
            if (layer.get('name') === baseLayerInput.value) {
                layerSwitcher.setVisible_(layer, 1);
            }
        });
    }, false);
    ...
    map.on('change:layers', function () {
        ol.control.LayerSwitcher.forEachRecursive(map, function (l) { // l, i, a
            if (!(l instanceof ol.layer.Group) && l.getVisible()) {
                if (l.get('type') === 'base') {
                    imageryLayerActions(l);
                } else if (l.get('type') === 'vector') {
                    vectorLayerActions(l);
                } else {
                    selectedLayerActions(l);
                }
            }
        });
    });
