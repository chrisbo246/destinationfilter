...
function updateScores() {
    ...
    if ($.isArray(profile.userReadingLanguages) && languagesDatas) {
    ...
    if ($.isArray(profile.userCurrencies)) {
        $.each(profile.userCurrencies, function (i, currencyCode) {
            if (currenciesDatas[currencyCode] && currenciesDatas[currencyCode].eurExchangeRate && (!profile.userChangeRate || currenciesDatas[currencyCode].eurExchangeRate < profile.userChangeRate)) {
                profile.userChangeRate = parseFloat(currenciesDatas[currencyCode].eurExchangeRate); // .replace(/[, ]/g, '')
            }
        });
    }
    ...
    if (countriesDatas) {
        $.each(countriesDatas, function (cid, country) {
            ...
            if (profile.userCurrencies && profile.userChangeRate) {
                ...
                if (currenciesDatas && currenciesDatas[country.currency.CurrencyCode] && currenciesDatas[country.currency.CurrencyCode].eurExchangeRate) {

                    // Country official currency value in euro (low rate)
                    countryChangeRate = parseFloat(currenciesDatas[country.currency.CurrencyCode].eurExchangeRate); // .replace(/[, ]/g, '')
            ...
            // Better life index
            $.each(prioritiesDatas, function (i, priorityDatas) {

                var indicator = priorityDatas.bliIndicator;
                var pid = priorityDatas.id;
                if (indicator && pid) {                    
                    // if (priorities[priorityDatas.id]) {
                    ...
                    if (country.betterLifeIndexes) {
                        $.each(country.betterLifeIndexes, function (i, bli) {
                            if (bli.INDICATOR === indicator && bli.INEQUALITY === 'TOT') {
                                ...
                                // if (country.ISO === 'FR') {console.warn('matchingPriorities['+pid+'].values.push('+bli.Value+')');}
                            }
                        });
                    } else {
        ...
        $.each(array, function (i, pid) {
            ...
            if (prioritiesDatas) {
                ...
                console.warn('prioritiesDatas global variable is not defined'); /* RemoveLogging:skip */
            }
            // if (pid === 'acVoltagesPriority') console.info('priorities['+pid+']='+JSON.stringify(enabledPriorities[pid]));
            ...
            $.each(countriesDatas, function (cid, country) {
                ...
                if (pid === 'acVoltagesPriority' && cid === 'au') console.info('countriesDatas['+cid+'].matchingPriorities['+pid+']='+JSON.stringify(country.matchingPriorities[pid]));
            ...
            if ((enabledPriorities[pid].points || enabledPriorities[pid].points === 0)
                ...
                $.each(countriesDatas, function (cid, country) {
                    ...
                    // if (pid === 'acVoltagesPriority' && cid === 'au') console.info('countriesDatas['+cid+'].matchingPriorities['+pid+'].values='+JSON.stringify(country.matchingPriorities[pid].values));
                    ...
                    if (country.matchingPriorities[pid] && country.matchingPriorities[pid].values && country.matchingPriorities[pid].values.length > 0) {
                        ...
                        if (pid === 'acVoltagesPriority' && cid === 'au') console.info('countriesDatas['+cid+'].matchingPriorities['+pid+'].points='+country.matchingPriorities[pid].points);
                        if (pid === 'acVoltagesPriority' && cid === 'au') console.info('countriesDatas['+cid+'].matchingPriorities['+pid+'].percent='+country.matchingPriorities[pid].percent);

                        // Add points to the country total
                        country.points = country.points + points;                        
                        if (pid === 'acVoltagesPriority' && cid === 'au') console.info('countriesDatas['+cid+'].points='+countriesDatas[cid].points);
        ...
        $.each(countriesDatas, function (cid, country) {
            ...
            if (cid === 'au') console.info('countriesDatas['+cid+'].percent='+countriesDatas[cid].percent);
        });
        
        
        
        
        deferred.ready.scores.resolve();
        
    } else {
        console.warn('countriesDatas variable is not defined'); /* RemoveLogging:skip */
    }    
