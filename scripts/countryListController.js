...
var countryListModule = (function () {
    ...
    var _drawCountryList = function () {
        ...
        sorted = [];
        $.each(countriesData, function (cid, v) {
            sorted.push({
                id: cid,
                row: i,
                points: v.points
        ...
        $.each(sorted, function (i, v) {
            ...
            // if (countriesData[v.id].points > 0 && maxPoints
            // && countriesData[v.id].maxPoints && countriesData[v.id].maxPoints > 0
            if (!options || !options.resultLimit || i < options.resultLimit) {

                countryData = countriesData[v.id];
                ...
                // data.countryName = 'values.countries.' + v.id.toUpperCase(); // i18nextInstance.t('country.' + v.id.toUpperCase()) // countryData.Country
    ...
    $(function () {
        ...
        $.when(deferred.getJSON.countriesData, deferred.ready.userProfile, deferred.ready.priorities).done(function () {//, deferred.ready.appSettings
