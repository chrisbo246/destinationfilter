...
var scoreModule = (function () {
    ...
    var updateScores = function () {
        ...
        $.when(deferred.getJSON.languagesData).done(function () {
        ...
        if ($.isArray(profile.userCurrencies)) {
            $.each(profile.userCurrencies, function (i, currencyCode) {
                if (currenciesData[currencyCode] && currenciesData[currencyCode].eurExchangeRate && (!profile.userChangeRate || currenciesData[currencyCode].eurExchangeRate < profile.userChangeRate)) {
                    profile.userChangeRate = parseFloat(currenciesData[currencyCode].eurExchangeRate); // .replace(/[, ]/g, '')
                }
            });
        }
        ...
        $.when(deferred.getJSON.countriesData).done(function () {
            $.each(countriesData, function (cid, country) {
                ...
                if (profile.userCurrencies && profile.userChangeRate) {
                    ...
                    if (currenciesData && currenciesData[country.currency.CurrencyCode] && currenciesData[country.currency.CurrencyCode].eurExchangeRate) {

                        // Country official currency value in euro (low rate)
                        countryChangeRate = parseFloat(currenciesData[country.currency.CurrencyCode].eurExchangeRate); // .replace(/[, ]/g, '')
                ...
                // Better life index
                $.when(deferred.getJSON.prioritiesData).done(function () {
            ...
            $.when(deferred.getJSON.prioritiesData).done(function () {
                ...
                $.each(array, function (i, pid) {
                    ...
                    $.each(countriesData, function (cid, country) {
                        ...
                        if (pid === 'acVoltagesPriority' && cid === 'au') console.info('countriesData['+cid+'].matchingPriorities['+pid+']='+JSON.stringify(country.matchingPriorities[pid]));
                    ...
                    if ((enabledPriorities[pid].points || enabledPriorities[pid].points === 0)
                        ...
                        $.each(countriesData, function (cid, country) {
                            ...
                            // if (pid === 'acVoltagesPriority' && cid === 'au') console.info('countriesData['+cid+'].matchingPriorities['+pid+'].values='+JSON.stringify(country.matchingPriorities[pid].values));
                            ...
                            if (country.matchingPriorities[pid] && country.matchingPriorities[pid].values && country.matchingPriorities[pid].values.length > 0) {
                                ...
                                if (pid === 'acVoltagesPriority' && cid === 'au') console.info('countriesData['+cid+'].matchingPriorities['+pid+'].points='+country.matchingPriorities[pid].points);
                                if (pid === 'acVoltagesPriority' && cid === 'au') console.info('countriesData['+cid+'].matchingPriorities['+pid+'].percent='+country.matchingPriorities[pid].percent);

                                // Add points to the country total
                                country.points = country.points + points;
                                if (pid === 'acVoltagesPriority' && cid === 'au') console.info('countriesData['+cid+'].points='+countriesData[cid].points);
                ...
                $.each(countriesData, function (cid, country) {
                    ...
                    if (cid === 'au') console.info('countriesData['+cid+'].percent='+countriesData[cid].percent);
                });
    ...
    */
    ...
    deferred.getJSON.prioritiesData = $.getJSON('modules/disabledPriorities/disabledPrioritiesModal.json', function (json) {
        prioritiesData = json;        
    ...
    deferred.getJSON.languagesData = $.getJSON('data/json/languages.json', function (json) {
        languagesData = json;
