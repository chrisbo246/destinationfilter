...
var mapModule = (function () {
    ...
    */
     ...
     */
    var fitCountry = function fitCountry(id) {
     ...
     */
    var destinationStyleFunction = function destinationStyleFunction(feature) {
        // feature, resolution
        var opacity,
            score = 0,
     ...
     */
    var missingDataStyleFunction = function missingDataStyleFunction(feature) {
        // feature, resolution
        var opacity,
     ...
     * Score layer style
     */
    var scoreStyleFunction = function scoreStyleFunction(feature, resolution) {
        var opacity, intensity, percent, missingDataPercent, id;
    ...
    var roadOverlays = new ol.layer.Group({
        name: 'roadOverlays',
        layers: [mapquestHybLayer, lonviaHikingLayer, lonviaCyclingLayer]
    });

    var informationOverlays = new ol.layer.Group({
        name: 'informationOverlays',
        layers: [timeZonesLayer, gpxFileLayer]
    });
    ...
    var roadBaselayers = new ol.layer.Group({
        name: 'roadBaselayers',
        layers: [openSeaMapLayer, openCycleMapLayer, mapquestOSMLayer, openStreetMapLayer]
    });

    var reliefBaselayers = new ol.layer.Group({
        name: 'reliefBaselayers',
        layers: [stamenTerrainWithLabelsLayer, arcgisLayer]
    });

    var aerialBaselayers = new ol.layer.Group({
        name: 'aerialBaselayers',
        layers: [mapquestSatLayer]
    });

    var baselayers = new ol.layer.Group({
        name: 'baseLayers',
        layers: [aerialBaselayers, reliefBaselayers, roadBaselayers]
    });

    var userOverlays = new ol.layer.Group({
        name: 'userOverlays',
        layers: [destinationsLayer, missingDataLayer, scoresLayer]
    });
    ...
    var layers = [baselayers, overlays
    // markerOverlays
    ];
    ...
    $(function () {
        ...
        $.when(appModule.deferred.ready.map).done(function () {
            ...
            $gpsMode.on('switchChange.bootstrapSwitch init', function () {
                // e, state
                if (this.checked) {
