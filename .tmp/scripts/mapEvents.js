...
var mapEventsModule = (function (mapModule) {
    ...
    var _wrapLon = function _wrapLon(value) {
    ...
    var _onMoveEnd = function _onMoveEnd(evt) {
        var map = evt.map,
    ...
    var _displayFeatureTooltip = function _displayFeatureTooltip(pixel) {
        //console.info('#map-tooltip 2:'+JSON.stringify(mapTooltip));
        mapTooltip.css({
            left: pixel[0] + 'px',
            top: pixel[1] - 15 + 'px'
        });
        var feature = mapModule.map.forEachFeatureAtPixel(pixel, function (feature) {
            // feature, layer
            return feature;
        });
        if (feature) {
            mapTooltip.tooltip('hide').attr('data-original-title', feature.get('name')).tooltip('fixTitle').tooltip('show');
        } else {
    ...
    */
    ...
    var _displayCountryDetails = function _displayCountryDetails(pixel) {
        // coordinate

        var feature;

        feature = mapModule.map.forEachFeatureAtPixel(pixel, function (feature) {
            // feature, layer
            return feature;
        });
