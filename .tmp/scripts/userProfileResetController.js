    ...
    $(function () {
        ...
        $('#user_profile_reset_modal').one('shown.bs.modal', function () {
            ...
            $.when(appModule.deferred.init.i18next).done(function () {
                ...
                // Field validation
                $form.bootstrapValidator($.extend({}, appModule.options.bootstrapValidator, {
                    fields: {
                        userProfile: {
                ...
                })).bootstrapValidator('validate').end().on('change', function () {
                    var $this = $(this);
                    if ($this.attr('id')) {
            ...
            // Focus select field
            $('#user_profile').focus().trigger('chosen:activate');
...
//# sourceMappingURL=userProfileResetController.js.map
