...
var disabledPrioritiesModule = (function () {
     ...
     */
    var _restoreCookies = function _restoreCookies() {
        ...
        if ($el) {
            $el.garlic({
                ...
                onPersist: function onPersist(elem, storedValue) {}
            });
        }
    ...
    var _disabledPrioritiesButtonState = function _disabledPrioritiesButtonState() {
        // state, futureState
        ...
        */
        ...
        $('#disabled_priorities_modal').one('shown.bs.modal', function () {
            ...
            if (!$el.mixItUp('isLoaded')) {
                ...
                $el.on('mixEnd', function () {
                    // e, state
                    _disabledPrioritiesButtonState();
                });

                // Filter selector (for select field only)
                $('#disabled_priorities_filters').val(options.mixitupPriorities.load.filter || 'all').on('change', function () {
                ...
                */
                ...
                // Layout selector
                $('#disabled_priorities_layout').on('change', function () {
        ...
        $el.sortable(options.sortable).disableSelection().on('sortstop sortreceive sortremove', function () {
            // event, ui
        ...
        $('#enable_selected_priorities').on('click', function (e) {
            e.preventDefault();
            var array = [];
            $.each($el.find('li.active:visible'), function () {
                // i, id
                $(this).removeClass('active').removeAttr('style').show();
        ...
        $.when(appModule.deferred.ready.enabledPriorities).done(function () {
            ...
            $('#enable_all_priorities').on('click', function (e) {
                e.preventDefault();
                var array = [];
                $.each($el.find('li:visible'), function () {
                    // i, id
                    $(this).removeClass('active').removeAttr('style').show();
                    $(this).find('.glyphicon-plus').removeClass('glyphicon-plus').addClass('glyphicon-remove');
                    array.push($(this));
            ...
            $el.on('click', '.bootstrap-switch-container', function (e) {
                // event, state
                e.preventDefault();
                e.stopPropagation();
            });
...
//# sourceMappingURL=disabledPrioritiesController.js.map
