            ...
            $.each(appModule.countriesData, function (cid, country) {
                ...
                if (profile.userCurrencies) {
                    ...
                    if (country.CurrencyCode) {
                        intersect = $.intersect(profile.userCurrencies, [country.CurrencyCode]).length;
                        if (intersect > 0) {
                ...
                if (profile.userSpeakingLanguages) {
                    if (country.languagePopulation) {
                        $.each(country.languagePopulation, function (j, language) {
                            intersect = $.intersect(profile.userSpeakingLanguages, [language.type]).length;
                            if (intersect > 0) {
                                country.matchingPriorities.speakingLanguagePriority.values.push(language.populationPercent);
                            } else {
                                country.matchingPriorities.speakingLanguagePriority.values.push(0);
                            }
                        });
                    } else {
                    ...
                    if (country.languagePopulation) {
                        $.each(country.languagePopulation, function (j, language) {
                            if (language.officialStatus) {
                                intersect = $.intersect(profile.userReadingScripts, [language.ScriptCode]).length;
                                if (intersect > 0 && $.inArray(language.officialStatus, ['official', 'de_facto_official', 'official_regional']) !== -1) {
                                    country.matchingPriorities.readingLanguagePriority.values.push(1); // .push(scoreValue)
                                }
                            }
                            /*else {
                ...
                if (profile.userPlugTypes) {
                    ...
                    if (country.plugs) {
                        $.each(country.plugs, function (i, plug) {
                            ...
                            if (plug.plugType) {
                                value = plug.plugType;
                                intersect = $.intersect(profile.userPlugTypes, [value]).length;
                                if (intersect > 0) {
                    ...
                    if (country.plugs) {
                        $.each(country.plugs, function (i, plug) {
                            ...
                            if (plug.frequencies) {
                                ...
                                intersect = $.intersect(profile.userPlugFrequencies.map(Number), values).length;
                ...
                if (profile.userDrivingSides) {
                    ...
                    if (country.drivingSide) {
                        intersect = $.intersect(profile.userDrivingSides, [country.drivingSide]).length;
                        if (intersect > 0) {
            ...
            $.when(prioritiesData).done(function () {
                ...
                $.each(array, function (i, pid) {
                    ...
                    $.each(appModule.countriesData, function (cid, country) {
                        ...
                        // && country.matchingPriorities[pid].values
                         && $.isArray(country.matchingPriorities[pid].values) && country.matchingPriorities[pid].values.length > 0) {
                            //  && country.matchingPriorities[pid].values.length > 0
                    ...
                    if ((enabledPriorities[pid].points || enabledPriorities[pid].points === 0) && (enabledPriorities[pid].maxValue || enabledPriorities[pid].maxValue === 0) && (enabledPriorities[pid].minValue || enabledPriorities[pid].minValue === 0)) {
