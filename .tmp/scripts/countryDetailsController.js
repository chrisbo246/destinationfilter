...
var countryDetailsModule = (function () {
     ...
     */
    var showCountryDetails = function showCountryDetails(cid, format) {
     ...
     */
    var _buildTemplate = function _buildTemplate(countryData) {
        ...
        if (countryData.currency.CurrencyCode && $('#user_currencies').val()) {
            ...
            $.each($('#user_currencies').val(), function (i, currencyCode) {
                var currencyName, localCurrencyValue, currencyData;
        ...
        $.each(array, function (i, pid) {
            ...
            if (countryData.matchingPriorities[pid]) {
                ...
                if (countryData.matchingPriorities[pid].required && countryData.matchingPriorities[pid].required.length > 0) {
                    data.warning = true;
                }
        ...
        $.get('templates/partials/countryDetails.hbs', function (source) {
            ...
            $('#country_details').find('.modal-dialog').html(template(viewData));
            /*.end()
...
//# sourceMappingURL=countryDetailsController.js.map
